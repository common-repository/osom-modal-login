		...
		?>
				...
				<div class="login-modal-box" id="OMLlogin">
					<div class="modal-content">
						...
						<?php if ( ! empty( $title ) ) : ?>
							<p class="modal-login-title"><?php echo esc_html( $title ); ?></p>
						<?php endif; ?>
						<?php
...
// Automatically add a login/logout block to the navigation block in block themes
add_filter( 'hooked_block_types', __NAMESPACE__ . '\add_loginout_navigation_block', 10, 4 );
function add_loginout_navigation_block( $hooked_block_types, $relative_position, $anchor_block_type, $context ) {

	if ( ! $context instanceof \WP_Post || 'wp_navigation' !== $context->post_type ) {
		return $hooked_block_types;
	}

	// Check if the loginout block is already present in the navigation block
	if ( str_contains( $context->post_content, '<!-- wp:loginout' ) ) {
		return $hooked_block_types;
	}

	// Add the loginout block to the navigation block
